...
class Input:
    def __init__(self, structure_loc, target, structure_name="", target_name="X", master_ali=""):
        ...
        for chain in self.structure_chains.keys():
            aa = list(self.structure[0][chain].get_residues())
            self.structure_chains[chain]["start"] = aa[0].full_id[-1][1]
            self.structure_chains[chain]["end"] = aa[-1].full_id[-1][1]

        # Writing of the PIR file; format:
        self.input_pir_loc = f"{self.structure_loc}/{self.target_name}.pir"
        self.pir_write()

        # Cleaning and preparing the PDB of the template
        self.input_pdb_loc = f"{self.structure_loc}/{self.structure_name}_cleaned.pdb"
        self.pdb_clean()
    ...
    def align(self, seq1, seq1_name, seq2_name, breaks):
        ...
        seq1_out = str([x for x in self.sequences if (seq1_name in x.name and seq2_name[0].lower() == x.name[-2])][0].seq)
    ...
    def pir_write(self):
        ...
        *
        ...
        out = []
        for chain in sorted(self.structure_chains.keys()):
            out.append("\n".join([f">P1;{self.structure_name}",
                                  ...
                                  *[self.structure_chains[chain]["sequence"][i:i+51] for i in range(0, len(self.structure_chains[chain]["sequence"]), 50)],
                                  "*"]))

            out.append("\n".join([f">P1;{self.target_name}",
                                  f"sequence:{self.target_name}:1:{chain}::{chain}:{self.target_name}:::",
                                  *[self.target_chains[chain]["sequence"][i:i+51] for i in range(0, len(self.target_chains[chain]["sequence"]), 50)],
                                  "*"]))
    ...
    def pdb_clean(self):
        ...
        selected_chains = _Select([x for x in self.structure[0].get_list() if x.id in self.structure_chains.keys()])
        io.save(self.input_pdb_loc, selected_chains)
